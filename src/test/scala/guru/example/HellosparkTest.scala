package guru.example
import org.apache.spark.sql.types._
import org.apache.spark.sql.{SparkSession, functions}
import org.scalatest.funsuite.AnyFunSuite

class SetSuite extends AnyFunSuite {
//class HellosparkTest extends FunSuite{
  var Age :Int = 5;
  test("testProductIdsCounts") {
    doConnectWithDatabase();
    countProductIds();
  }

  def doConnectWithDatabase(): Unit = {
    ...
    val df = spark.read
      ...
      .option("delimiter", "\t").option("header", "true").option("inferSchema", "false").load("/Users/JO20382899/Downloads/files-main/insight_data/products.csv")

    // Register the DataFrame as a global temporary view
    df.createGlobalTempView("products")
    val penCountGreatherThan5000: Int = 4;
    //Select and show all the records with quantity >= 5000 and name starts with 'Pen'
    var penCount: Long = spark.sql("SELECT * FROM global_temp.products WHERE quantity >= 5000 AND name LIKE 'Pen%' ").count();
    println("The pen count is : "+ penCount);
    assert(penCount == penCountGreatherThan5000);

    val priceLess :Int = 3;
   // Select and show all the records with quantity >= 5000, price is less than 1.24 and name starts with 'Pen'
   var priceCount: Long =  spark.sql("SELECT * FROM global_temp.products WHERE quantity >= 5000 AND price < 1.24 AND name LIKE 'Pen%' ").count();
   println("price count : "+priceCount)
    assert(priceCount == priceLess);
    ...
    val vdf = spark.read
      ...
      .schema(mdf)
      .option("delimiter", ",")
      .option("header", "true").option("multiline", "true").load("/Users/SU20370969/Downloads/files-main/insight_data/supplier.csv")
    ...
    val cvdf = spark.read
      ...
      .option("delimiter", ",")
      .option("header", "true").option("multiline", "true").load("/Users/SU20370969/Downloads/files-main/insight_data/products_supplier.csv")
